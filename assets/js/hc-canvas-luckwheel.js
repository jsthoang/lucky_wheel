...
(async function async() {
   ...
   await fetch("http://localhost:3000/gifts")
   ...
   await fetch("http://localhost:3000/prizes")
      .then((response) => response.json())
         ...
         gotBack: async function (data) {
            if (data == null) {
               ...
               if (data.search('.000') !== -1) {
                  ...
                  notification.push(`${user.name} vừa trúng lì xì trị giá ${data}VNĐ !!!`);
               } else {
               ...
               setTimeout(() => {
                  Promise.all([postGifts({ ...user, gift: data }), updateUse(user)]).then(document.querySelector('#timesSpin').innerHTML = user.timesSpin);
               }, 2000);
            }
         }
      });
   ...
   const randomIndex = async (prizes) => {
      ...
      await fetch("http://localhost:3000/gifts")
         ...
         .then(async (data) => {
            ...
            if (indexCheckGift !== -1) {
               return alert("Bạn đã hết lượt quay!");
            }
...
async function updateUse(user) {
   fetch(`http://localhost:3000/staff/${user.id}`, {
      method: 'PUT',
      headers: {
         'Content-Type': 'application/json',
      },
